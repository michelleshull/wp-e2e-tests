...
test.afterEach( function() {
	...
	if ( this.currentTest.state === 'failed' ) {
		...
		return driver.takeScreenshot().then( ( data ) => {
			driver.getCurrentUrl().then( url => {
				mediaHelper.writeScreenshot( data, function( date ) {
					return `FAILED-${locale}-${screenSize}-${shortTestFileName}-${date}`;
				}, {
					url
				} );
			} );
	...
	if ( config.get( 'saveAllScreenshots' ) === true ) {
		return driver.takeScreenshot().then( ( data ) => {
			driver.getCurrentUrl().then( url => {
				mediaHelper.writeScreenshot( data, function( date ) {
					return `${locale}-${screenSize}-${shortDescribeFileName}-${date}-${shortTestFileName}.png`;
				}, {
					url
				} );
			} );
