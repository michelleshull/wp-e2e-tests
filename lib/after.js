...
test.afterEach( function( done ) {
	...
	return driver.takeScreenshot().then(
		data => driver.getCurrentUrl().then(
			url => mediaHelper.writeScreenshot( data, filenameCallback, { url } )
		),
		err => slackNotifier.warn( `Could not take screenshot due to error: '${err}'` )
	).then( done );
} );
